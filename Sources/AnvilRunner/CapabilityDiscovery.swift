...
public actor CapabilityDiscovery {
    ...
    private func detectTools() async -> ToolCapabilities {
        return ToolCapabilities(
            swift: await detectTool(named: "swift", versionArgs: ["--version"]),
            xcode: await detectXcode(),
            git: await detectTool(named: "git", versionArgs: ["--version"]),
            githubCLI: await detectGitHubCLI()
        )
    }
    ...
    private func detectAgents() async -> AgentCapabilities {
        return AgentCapabilities(
            claude: await detectAgent(named: "claude"),
            codex: await detectAgent(named: "codex"),
            gemini: await detectAgent(named: "gemini")
        )
    }
    ...
    private func detectTailscale() async -> TailscaleInfo {
        ...
        let loggedIn: Bool
        if running {
            loggedIn = await isTailscaleLoggedIn(path: path)
        } else {
            loggedIn = false
        }
    ...
    private func makeCheck(id: String, category: String, condition: Bool, passMessage: String, failMessage: String) -> HealthCheck {
        HealthCheck(
            id: id,
            category: category,
    ...
    private func makeCheck(id: String, category: String, condition: Bool, passMessage: String, warnMessage: String) -> HealthCheck {
        HealthCheck(
            id: id,
            category: category,
    ...
    private func parseVersion(from output: String, for tool: String) -> String? {
